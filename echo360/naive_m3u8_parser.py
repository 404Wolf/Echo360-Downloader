...
class NaiveM3U8Parser:
    ...
    @staticmethod
    def _extract_properties(lines: str, index: int):
        assert lines[index].startswith("#")
        tokens = NaiveM3U8Parser._tokenise(NaiveM3U8Parser._remove_prefix(lines[index]))
        properties = {}
        ...
        if "URI" in tokens.keys():
            ...
            properties["URI"] = lines[index + 1].strip()
    ...
    @staticmethod
    def _split_on_comma_unless_inside_quotes(string: str):
    ...
    @staticmethod
    def _remove_prefix(string: str):
        matches =re.search("(?:[#a-zA-Z-]+:)(.*)$", string)
        if matches is None:
